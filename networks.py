...
class ConvEncoder(nn.Module):
    ...
    def forward(self, obs):
        ...
        x = obs.reshape((-1,) + tuple(obs.shape[-3:])) # torch.Size([1024, 64, 64, 3])
        # (batch * time, h, w, ch) -> (batch * time, ch, h, w)
        x = x.permute(0, 3, 1, 2) # torch.Size([1024, 3, 64, 64])
        x = self.layers(x) # torch.Size([1024, 256, 4, 4])
        # (batch * time, ...) -> (batch * time, -1)
        x = x.reshape([x.shape[0], np.prod(x.shape[1:])]) # torch.Size([1024, 4096])
...
class mocoEncoder(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Linear(4096, dim_half)
        self.fc2 = nn.Linear(4096, dim_half)
        self.activation = nn.ReLU(inplace=True)
        self.fc3 = nn.Linear(dim_half * dim_half, 4096)
